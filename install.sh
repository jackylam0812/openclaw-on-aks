...
deploy_portals() {
  ...
  sed "s|PORTAL_API_IMAGE|$PORTAL_API_IMAGE|g; s|JWT_SECRET_VALUE|$JWT_SECRET|g; s|LITELLM_API_KEY_VALUE|$LITELLM_KEY|g; s|LITELLM_MASTER_KEY_VALUE|$MASTER_KEY|g; s|API_BASE_URL_VALUE|http://localhost:3000|g; s|ACS_SENDER_ADDRESS_VALUE|DoNotReply@openclaw-azure.org|g" \
  ...
  sed "s|PORTAL_API_IMAGE|$PORTAL_API_IMAGE|g; s|JWT_SECRET_VALUE|$JWT_SECRET|g; s|LITELLM_API_KEY_VALUE|$LITELLM_KEY|g; s|LITELLM_MASTER_KEY_VALUE|$MASTER_KEY|g; s|API_BASE_URL_VALUE|$API_BASE|g; s|ACS_SENDER_ADDRESS_VALUE|DoNotReply@openclaw-azure.org|g" \
    portals/k8s/api-deployment.yaml | kubectl apply -f -
